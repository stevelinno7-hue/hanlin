(function(global){
    ...
    function init() {
        ...
        grades.forEach(grade => {
            // 1. 物理隔離：只篩選出該年級的資料
            const gradePool = geoDB.filter(q => q.t[0] === grade);

            if (gradePool.length > 0) {
                
                // 模板 A: 特徵題 (只註冊該年級)
                G.registerTemplate(`geo_feat_${grade}`, (ctx, rnd) => {
                    const item = pick(gradePool); // 只從該年級池抓題
                    
                    // 誘答：優先找同領域的錯誤選項 (也限制在該年級)
                    const sameTag = gradePool.filter(x => x.t[1] === item.t[1] && x.e !== item.e);
                    const others = gradePool.filter(x => x.e !== item.e);
                    let wrongOpts = [];
                    if (sameTag.length >= 3) wrongOpts = shuffle(sameTag).slice(0, 3).map(x => x.y);
                    else wrongOpts = shuffle(others).slice(0, 3).map(x => x.y);
                ...
                }, ["geography", "地理", grade]); // 只給「地理」和「該年級」標籤

                // 模板 B: 關鍵字題
                G.registerTemplate(`geo_key_${grade}`, (ctx, rnd) => {
                    const item = pick(gradePool);
                    const opts = shuffle([item.k, ...shuffle(gradePool.filter(x=>x!==item)).slice(0,3).map(x=>x.k)]);
